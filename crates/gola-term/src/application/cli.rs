...
fn format_session(session: &Session) -> String {
    let mut res = format!("- (ID: {}) {}", session.id, session.timestamp);
...
async fn load_config_from_session_interactive() -> Result<()> {
    ...
    load_config_from_session(&sessions[idx.unwrap()].id).await?;
...
fn subcommand_debug() -> Command {
    ...
    cmd = cmd.about("Debug helpers for gola")
        .hide(true)
        .subcommand(
        ...
        .subcommand(
            Command::new("log-path").about("Output path to debug log file generated when running gola with environment variable RUST_LOG=gola")
        )
        .subcommand(
...
fn subcommand_chat() -> Command {
    return Command::new("chat").about("Start a new chat session.");
}
    ...
    return Command::new("gola")
        ...
        .arg(
            Arg::new(ConfigKey::Editor.to_string())
                ...
                .num_args(1)
                .help(format!(
                    "The editor to integrate with. [default: {}]",
                    Config::default(ConfigKey::Editor)
                ))
        ...
        .arg(
            Arg::new(ConfigKey::GolaAgUIURL.to_string())
                .long(ConfigKey::GolaAgUIURL.to_string())
                .env("GOLA_AG_UI_URL")
                .num_args(1)
                .help(format!(
                    "Gola Agent UI URL when using the Gola backend. [default: {}]",
                    Config::default(ConfigKey::GolaAgUIURL)
                ))
                .global(true),
        );
}
    ...
    match matches.subcommand() {
        Some(("debug", debug_matches)) => {
            match debug_matches.subcommand() {
                ...
                Some(("log-path", _)) => {
                    let log_path = dirs::cache_dir().unwrap().join("gola/debug.log");
