...
use crate::infrastructure::clients::AgentClientManager;
use crate::infrastructure::editors::EditorManager;

fn is_line_width_sufficient(line_width: u16) -> bool {
    let author_lengths = vec![Author::User, Author::Gola, Author::Model]
        .into_iter()
    ...
    trimmed_line_width >= min_width
}

pub async fn start_loop<B: ratatui::backend::Backend>(
    ...
    loop {
        ...
        macro_rules! send_user_message {
            ( $input_str:expr ) => {
                ...
                let mut prompt =
                    AgentPrompt::new(input_str.to_string(), app_state.agent_context.clone());

                if app_state.agent_context.is_empty() && SlashCommand::parse(&input_str).is_none() {
        ...
        match events.next().await? {
            ...
            Event::AgentPromptResponse(msg) => {
                app_state.handle_agent_response(msg.clone());
                if msg.done {
                    app_state.save_session().await?;
                }
            }
...
pub async fn start(
    ...
    let agent = AgentClientManager::get(AgentName::GolaAgUI)?;
