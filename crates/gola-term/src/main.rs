//! Terminal user interface for interactive agent conversations
//!
//! This binary provides a rich terminal experience for interacting with Gola agents,
//! prioritizing developer ergonomics and keyboard-driven workflows. The design choice
//! of a TUI over web interfaces reflects the target audience of developers who value
//! efficiency and integration with terminal-based toolchains. Features like syntax
//! highlighting, markdown rendering, and clipboard integration make this the preferred
//! interface for technical users who spend their time in the terminal.
...
use domain::models::AgentName;
use domain::models::Event;
use domain::services::clipboard::ClipboardService;
use infrastructure::clients::AgentClientManager;
use tokio::sync::mpsc;
use tokio::task;
use yansi::Paint;
...
fn handle_error(err: Error) {
    eprintln!(
            "{}",
            Paint::red(format!(
                "Oh no! Gola has failed with the following app version and error.\n\nVersion: {}\nCommit: {}\nError: {}",
    ...
    if backtrace.to_string() == "disabled backtrace" {
        ...
        eprintln!(
            "\nhttps://github.com/gbrigandi/gola/blob/v{}/README.md#report-an-issue",
            env!("CARGO_PKG_VERSION")
        );
...
async fn main() {
    ...
    let debug_log_dir = env::var("GOLA_LOG_DIR").unwrap_or_else(|_| {
        dirs::cache_dir()
            .unwrap()
            .join("gola")
            .to_string_lossy()
    ...
    background_futures.spawn(async move {
        ActionsService::start(
            AgentClientManager::get(AgentName::GolaAgUI).unwrap(),
            action_tx_clone,
            event_tx,
            &mut action_rx,
        )
