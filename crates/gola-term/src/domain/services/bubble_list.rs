    ...
    pub fn set_messages(&mut self, messages: &[Message], line_width: usize) {
        ...
        self.lines_len = messages
            ...
            .map(|(idx, message)| {
                ...
                // Convert lines to static by converting spans to owned strings
                let static_lines: Vec<Line<'static>> = bubble_lines.into_iter().map(|line| {
                    let static_spans: Vec<ratatui::text::Span<'static>> = line.spans.into_iter().map(|span| {
