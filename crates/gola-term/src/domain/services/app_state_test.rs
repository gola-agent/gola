...
use crate::domain::models::AgentClient;
...
mod handle_slash_commands {
    ...
    fn it_returns_error_message_on_invalid_codeblock() -> Result<()> {
        ...
        assert!(should_continue);
        assert!(!app_state.waiting_for_backend);
        assert_eq!(last_message.author, Author::Gola);
...
mod handle_agent_response {
    use super::*;

    #[test]
    fn it_handles_new_agent_response() {
        let mut app_state = AppState::default();
        app_state
            .messages
        ...
        app_state.handle_agent_response(agent_response);

        assert_eq!(app_state.messages.len(), 2);
    }

    #[test]
    fn it_handles_bad_agent_response() {
        let mut app_state = AppState::default();
        app_state
            .messages
            .push(Message::new(Author::User, "Do something for me!"));
    ...
    #[test]
    fn it_handles_icebreaker_message() {
        let mut app_state = AppState::default();
        let agent_response = AgentResponse {
            author: Author::Model,
            text: "Hey there! What can I do for you?".to_string(),
            done: true,
            context: Some("icebreaker_context".to_string()),
        };
        app_state.handle_agent_response(agent_response);

        assert_eq!(app_state.messages.len(), 1);
        assert_eq!(app_state.messages[0].author, Author::Model);
        assert_eq!(
...
mod init {
    ...
    async fn it_inits_and_reloads_from_session() -> Result<()> {
        let agent_client = Box::new(MockAgentClient {});
