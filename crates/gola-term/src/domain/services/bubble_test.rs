...
#[test]
fn it_creates_author_gola_text() -> Result<()> {
    let lines_str = create_lines(Author::Gola, BubbleAlignment::Left, 0, "Hi there!")?;
...
#[test]
fn it_creates_author_gola_text_long() -> Result<()> {
    let lines_str = create_lines(Author::Gola, BubbleAlignment::Left, 0, "Hi there! This is a really long line that pushes the boundaries of 50 characters across the screen, resulting in a bubble where the line is wrapped to the next line. Cool right?")?;
    insta::assert_snapshot!(lines_str, @r###"
    ╭Gola─────────────────────────────────────────╮
