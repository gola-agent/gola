...
rm -rf manpages && mkdir manpages && ./target/debug/gola-term manpages | gzip -c -9 >manpages/gola-term.1.gz
cargo xtask update-readme
rm -f config.example.toml
./target/debug/gola-term config default >config.example.toml
cargo bin mise x -C tools/node -- npm run prettier-write
...
sleep 5
while true; do
	res=$(cargo gha gh run list -R dustinblackman/gola-term -w build --json conclusion,databaseId | jq -rc '.[0]')
	echo "Status: $res"
...
rm -rf dist-gh
mkdir dist-gh
export GH_RUN_ID=$(cargo gha gh run list -R dustinblackman/gola-term -w build --json databaseId | jq -rc '.[0].databaseId')
