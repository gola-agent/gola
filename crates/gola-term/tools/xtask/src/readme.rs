...
fn cmd(args: Vec<&str>) -> String {
    let mut child = process::Command::new("./target/debug/gola");
    for arg in args {
        child.arg(arg);
    }

    for (key, _) in env::vars() {
        if key.starts_with("GOLA_") {
            child.env(key, "");
        }
    }
...
pub fn update() {
    ...
    readme.replace_range(
        ...
        &format!(
            "<!-- choco-install start -->\n```sh\nchoco install gola-term --version={version}\n```\n"
        ),
    );
...
curl -L -o gola-term.apk "https://github.com/gbrigandi/gola/crates/gola-term/releases/download/v{version}/gola-term_{version}_linux_${{arch}}.apk"
apk add --allow-untrusted ./gola-term.apk
```
"#)
    );
