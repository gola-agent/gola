...
fn get_syntaxes(syntaxes: Vec<Asset>) -> Result<()> {
    ...
    if let Ok(env_syntaxes_dir) = env::var("GOLA_BUILD_DOWNLOADED_SYNTAXES_DIR") {
    ...
    println!(
        "cargo:rustc-env=GOLA_SYNTAX_BIN={}",
    ...
    // syntaxes.
    if env::var("GOLA_BUILD_DOWNLOADED_SYNTAXES_DIR").is_err() {
...
fn get_themes(themes: Vec<Asset>) -> Result<()> {
    ...
    if let Ok(env_themes_dir) = env::var("GOLA_BUILD_DOWNLOADED_THEMES_DIR") {
    ...
    println!(
        "cargo:rustc-env=GOLA_THEMES_BIN={}",
    ...
    if env::var("GOLA_BUILD_DOWNLOADED_THEMES_DIR").is_err() {
