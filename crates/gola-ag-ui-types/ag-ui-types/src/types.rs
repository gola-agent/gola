...
pub struct RunAgentInput {
    ...
    /// Tool authorizations provided by the client for pending tool calls.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub tool_authorizations: Vec<ToolAuthorization>,
}

/// Represents a user's authorization for a specific tool call.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct ToolAuthorization {
    /// The ID of the tool call being authorized.
    #[serde(rename = "toolCallId")]
    pub tool_call_id: String,
    /// Whether the tool call is authorized by the user.
    pub authorized: bool,
    /// Optional reason for denial, or modified arguments if allowed by the system.
    /// For now, this is a placeholder and not actively used by the agent logic.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub notes: Option<String>,
}

impl ToolAuthorization {
    /// Creates a new tool authorization.
    pub fn new(tool_call_id: String, authorized: bool) -> Self {
        Self {
            tool_call_id,
            authorized,
            notes: None,
...
impl RunAgentInput {
    ...
    pub fn new(
        ...
        tool_authorizations: Vec<ToolAuthorization>, // Added field
    ) -> Self {
        Self {
            thread_id,
            run_id,
            ...
            context,
            forwarded_props,
            tool_authorizations, // Added field
        }
    }
}
